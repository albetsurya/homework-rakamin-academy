...
const logTxt = `03/22 08:54:24 INFO   :......rsvp_flow_stateMachine: state RESVED, event T1OUT
...
This is additional data that will be appended to the log.txt file.`;
...
fs.appendFile("log.txt", logTxt, (err) => {
  if (err) {
    console.error("Error appending data to log.txt:", err);
  } else {
    console.log("New data has been appended to the log.txt file");
  }
});
